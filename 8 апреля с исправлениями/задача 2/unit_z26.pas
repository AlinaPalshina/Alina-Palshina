...
begin
  ...
  if TryStrToFloat(Edit_r.text,r) = true then
  begin
     Edit_r.Color:=clWhite;
     exit;
  end;
...
begin
  ...
  begin
    DataFromForm();
    save_params(r,fi, SaveDialog1.FileName);
  end;
end;
...
begin
  ...
  begin
    load_params(r, fi, SaveDialog1.FileName);
    Edit_r.Text:=floattostr( r );
    Edit_d.Text:=floattostr( fi );
  end;
end;
